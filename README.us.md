...
- Service API contract (v1): `docs/architecture/SERVICE_API_CONTRACT_V1.md`.
...
| `LOG_FILE` | `wifi_optimizer.log` | Path to the log file. Relative to the working directory by default. |
...
## 🔧 Service API (programmatic use)

The optimizer core is available as a Python API without any CLI dependencies — designed for the Windows service wrapper to call directly.

```python
from wifi_optimizer.config import OptimizerConfig
from wifi_optimizer.service_api import OptimizationService

# Configure programmatically (no .env or CLI args needed)
config = OptimizerConfig(
    router_url="http://192.168.100.1",
    router_user="admin",
    router_pass="my_password",
)

# Or load from environment variables / .env
config = OptimizerConfig.from_env()

service = OptimizationService(config)
result  = service.run_cycle(dry_run=True)   # True = no router changes

print(result.status)    # "success" | "no_change" | "error"
print(result.changed)   # True if a channel change was applied
print(result.to_dict()) # JSON-serializable dict — contract v1
```

`OptimizationResult` never calls `sys.exit()`. On failure it returns `status="error"` with a structured `details` field, making it safe to embed in long-running services.

The full payload contract is defined in `docs/architecture/SERVICE_API_CONTRACT_V1.md`.
...
├── wifi_optimizer/
│   ├── config.py                  # OptimizerConfig dataclass — typed configuration schema
│   ├── service_api.py             # OptimizationService — programmatic API without CLI deps
...
│   ├── analyzer.py                # Window analysis: reads DB, writes optimal_windows.json
│   └── routers/
...
├── tests/
│   └── test_service_api.py        # Unit tests for OptimizationService
├── analyze_windows.py             # Standalone convenience wrapper for --analyze
...
└── optimal_windows.json           # Optimal hourly windows generated by --analyze (git-ignored)
```
